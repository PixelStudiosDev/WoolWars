...
public class DamageListener implements Listener {
    ...
    public void onDamage(EntityDamageEvent event) {
        ...
        if (event instanceof EntityDamageByEntityEvent) {
            ...
            switch (damageEvent.getEntityType()) {
                case PRIMED_TNT:
                    TNTPrimed tnt = (TNTPrimed) damageEvent.getDamager();
                    if (tnt.hasMetadata("woolwars")) {
                        Vector velocity = player.getLocation().toVector().subtract(tnt.getLocation().toVector()).normalize();
                        velocity.multiply(tnt.getMetadata("woolwars").get(0).asDouble());
                    ...
                    break;
                case PLAYER:
                    WoolPlayer damager = PlayerManager.get((Player) damageEvent.getDamager());
                    if (!damager.isAlive() || arena.isTeammate(woolPlayer, damager)) {
                        event.setCancelled(true);
                        return;
                    }
                    break;
                default:
                    if (damageEvent.getDamager() instanceof Projectile) {
                        ...
                        if (projectile.getShooter() instanceof Player) {
                            ...
                            if (arena.isTeammate(woolPlayer, shooter)) {
                                event.setCancelled(true);
                                return;
                            }
                        }
        ...
        // Check whether the player is supposed to die
        if (((player.getHealth() - event.getFinalDamage()) <= 0)) {
            ...
            if (event instanceof EntityDamageByEntityEvent) {
                ...
                if (damageEvent.getDamager() instanceof Player) {
                    WoolPlayer damager = PlayerManager.get((Player) damageEvent.getDamager());
                    Events.call(new PlayerKillEvent(arena, damager, woolPlayer, PlayerKillEvent.KillCause.PVP));
                    return;
                }
                ...
                switch (event.getCause()) {
                    ...
                    case PROJECTILE:
                        Projectile projectile = (Projectile) damageEvent.getDamager();
                        if (projectile.getShooter() instanceof Player) {
                            WoolPlayer damager = PlayerManager.get((Player) projectile.getShooter());
                            Events.call(new PlayerKillEvent(arena, damager, woolPlayer, PlayerKillEvent.KillCause.PROJECTILE));
                        } else {
                            Events.call(new PlayerKillEvent(arena, null, woolPlayer, PlayerKillEvent.KillCause.PROJECTILE));
                        }
                        break;
                    case FALL:
    ...
    public static void handleDeath(WoolPlayer player, Arena arena) {
        ...
        player.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, 50, 0, false, false));
