 ...
 * @author kangarko and Tvhee (https://github.com/Tvhee-Dev)
 */
public class SimpleScoreboard
{
	 ...
	 */
	private final List<Player> scoreboards = new ArrayList<>();
	 ...
	 */
	public SimpleScoreboard(Plugin plugin, String title, int updateDelayTicks)
	{
		setTitle(title);
	...
	public final void show(final Player player)
	{
		if(this.title == null || this.title.isEmpty())
			throw new IllegalArgumentException("Before calling show(Player) you need to use setTitle() for " + this);

		if(this.isViewing(player))
			throw new IllegalArgumentException("Player " + player.getName() + " is already viewing scoreboard: " + this.getTitle());
	...
	{
		if(!this.isViewing(player))
			throw new IllegalArgumentException("Player " + player.getName() + " is not viewing scoreboard: " + this.getTitle());
	...
	{
		entries.forEach(entry -> this.rows.add(ChatColor.translateAlternateColorCodes('&', entry)));
	}
	...
	{
		if(this.updateTask != null)
			throw new IllegalArgumentException("Scoreboard " + this + " already running");
		...
		this.updateTask = new BukkitRunnable()
		{
			...
			public void run()
			{
				...
				catch(final Throwable t)
				{
					...
					throw new IllegalArgumentException("Error displaying " + SimpleScoreboard.this + "Entries: " + lines + "Stopping rendering for safety.", t);
				}
			}
		}.runTaskTimer(plugin, 0, this.updateDelayTicks);
	}
	...
	private void update()
	{
		...
		for(final Player viewer : scoreboards)
		{
			if(!viewer.isOnline())
			{
	...
	{
		if(this.updateTask == null)
			throw new IllegalArgumentException("Scoreboard " + this + " not running");
	...
	public static List<String> passMergeThrough(String marge, List<String> text, int maxLength)
	{
		...
		for(int index = 0; index < stringLengths.size(); index++)
		{
			...
			final String subString =
					textWithMarge.substring(previousLength, Math.min(textWithMarge.length(), previousLength + length));
